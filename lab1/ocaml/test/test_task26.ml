...
let test_cycle_length () =
  ...
  check int "1/6 has cycle length 1" 1 (cycle_length 6) ;  (* 0.1(6) - цикл "6" *)
  check int "1/7 has cycle length 6" 6 (cycle_length 7) ;
  check int "1/9 has cycle length 1" 1 (cycle_length 9) ;
  check int "1/2 has cycle length 0" 0 (cycle_length 2) ;  (* 0.5 - без цикла *)
  check int "1/4 has cycle length 0" 0 (cycle_length 4)    (* 0.25 - без цикла *)
...
let () =
  ...
  run "task26"
    [ ("cycle_length", [test_case "Calculate cycle length" `Quick test_cycle_length])
